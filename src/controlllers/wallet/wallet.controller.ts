...
export class WalletController {
    ...
    @Post('payment')
    async proxyInitiatePayment(
       @Request() req: any,
        @Body() paymentData: any
    ) {
        try {
            console.log('Payload recibido red:', req.user);
            return await this.walletService.initiatePayment(paymentData, req.user.customer);
        } catch (error) {
            console.log(error);
            throw new HttpException('Error al procesar el pago', HttpStatus.BAD_GATEWAY);
        }
    }

    @Post('payment/confirm')
    async confirmPayment(@Body() confirmData: { sessionId: string; confirmationCode: string }) {
    ...
    async getBalance(@Request() req: any) {
        ...
        try {
            const result = await this.walletService.getBalanceByDocumentAndPhone(document, phone);
            return {
                success: true,
